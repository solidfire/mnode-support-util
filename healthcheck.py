...
class HealthCheck():
    ...
    def service_uptime(repo):
        logmsg.info("\nChecking service uptimes")
        minute = 0
        second = 0
        ps = DockerInfo.docker_ps(repo)
        for line in ps:
            if 'minutes' in line or 'minute' in line:
                minute += 1
            if 'seconds' in line:
                second += 1
        if minute != (len(ps) - 1) or second != (len(ps) - 1) and second < minute:
            logmsg.info("\t1 or more services have short Uptimes. Services may be restarting. Check sudo docker ps and service log")
        else:
            logmsg.info("\tAll services show about the same uptime")
